...
@XmlType(name = "rl01670DTO", propOrder = { "modifyDateStart", "modifyDateEnd", "modifyBatch", "modifyKind",
        "reportType", "reportURL" })
@XmlRootElement(name = "Rl01670DTO")
public class Rl01670DTO implements Serializable {

    /**
     * 
     */
    private static final long serialVersionUID = -3289478647845585778L;

    /** 編釘日期(起) */
    @FieldName("編釘日期(起) ")
    @XmlElement(name = "ModifyDateStart", required = false)
    private String modifyDateStart;

    /** 編釘日期(迄) */
    @FieldName("編釘日期(迄) ")
    @XmlElement(name = "ModifyDateEnd", required = false)
