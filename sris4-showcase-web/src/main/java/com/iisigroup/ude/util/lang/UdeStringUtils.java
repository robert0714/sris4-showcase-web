...
public final class UdeStringUtils {
     ...
     *
     * @param codePointArray UNICODE-Integer 陣列
     * @return 字串
     */
    @FunctionCode("字碼陣列轉換")
    ...
    enum Full2HelfMapping {
        ...
        //        U+201B  SINGLE HIGH-REVERSED-9 QUOTATION MARK (U+201B)  ‛   arial_unicode_ms
        ...
        //        U+201F  DOUBLE HIGH-REVERSED-9 QUOTATION MARK (U+201F)  ‟   arial_unicode_ms
        ...
        //        U+2039  SINGLE LEFT-POINTING ANGLE QUOTATION MARK (U+2039)  ‹   arial_unicode_ms
        //        U+203A  SINGLE RIGHT-POINTING ANGLE QUOTATION MARK (U+203A)     ›   arial_unicode_ms
        ...
        static {
            Full2HelfMapping[] values = values();
            for (int i = 0; i < CJK_Symbols_MAPPING.length; i++) {
                CJK_Symbols_MAPPING[i] = i + 0x3000; // TODO ? OR ' ' OR originalCode
            }
            for (Full2HelfMapping mapping : values) {
     ...
     * 全型字串轉半型.
     *
     * ! 情境還不夠完整，其它特殊字碼區如何對應，相關需求沒有收集完整。
     * ! TODO 也許未來會定義相關設定檔給開發者自行決定。
     *
     * @return 半型文字
     */
    @FunctionCode("全半型轉換")
     ...
     * U+ FFFE 不存在 Unicode中.
     * U+ 2060 Unicode 3.2 新增，以代替 U+FEFF 字元廢止之零寬不換行空格功能
     *
     * @param srcString 原始資料
     * @return 清除後的資料
     */
    @FunctionCode("刪減")
